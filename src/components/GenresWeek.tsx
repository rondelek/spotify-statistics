...
export default function GenresWeek() {
  let dataArtistsWeek: [] | null = null;
  let genresArray: any = [];
  let genresDict = {};
  ...
  useEffect(() => {
    const dataArtists = localStorage.getItem("dataArtistsWeek");
    if (dataArtists) {
      dataArtistsWeek = JSON.parse(dataArtists);
    }
  }, []);
